...
def fetch_url(url):
    ...
    session.headers.update({"User-Agent": "aip-extract/1.0 https://github.com/tsia/aip-extract"})
...
def fetch_folder(foldertitle, folderurl, depth=0):
    ...
    for item in items:
        if "folder-link" in item["class"]:
            ...
            print(title)
            fetch_folder(
                title, requests.compat.urljoin(BASE_URL, item.get("href")), depth + 1
            )
...
def generate_pdf(title):
    ...
    layout = img2pdf.get_fixed_dpi_layout_fun(
        (150, 150)
    )  # Force 150dpi which results in A5
